...
public class TicketController {
    ...
    public ResponseEntity<ApiResponse<Page<TicketDTO>>> getAll(
        ...
        return ResponseEntity.ok(ApiResponse.success("Tickets fetched", ticketService.getAllTickets(filter, pageable)));
    }
    ...
    public ResponseEntity<ApiResponse<TicketDTO>> getByNumber(@PathVariable String ticketNumber) {
        return ResponseEntity.ok(ApiResponse.success("Ticket fetched", ticketService.getTicketByNumber(ticketNumber)));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<TicketDTO>> create(@Valid @RequestBody TicketDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("Ticket created successfully", ticketService.createTicket(dto)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<TicketDTO>> update(@PathVariable Long id, @Valid @RequestBody TicketDTO dto) {
        return ResponseEntity.ok(ApiResponse.success("Ticket updated successfully", ticketService.updateTicket(id, dto)));
    }
